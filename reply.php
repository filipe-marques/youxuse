...
// name of user
$nome_user_message = mysql_escape_string(htmlspecialchars(trim($_GET['user']), ENT_QUOTES));

if (isset($_POST['submensagem'])) {
    // id of annoucement
    $query = mysql_escape_string(htmlspecialchars(htmlentities(trim($_GET['hut'])), ENT_QUOTES));

    // id of user
    $id = mysql_escape_string(htmlspecialchars(htmlentities(trim($_GET['ddk'])), ENT_QUOTES));
    ...
    if (strlen($mens) > 120) {
        ...
        echo ("" . LABEL_CONTACTAR_TEXT1 . " " . strlen($mens) . " " . LABEL_CONTACTAR_TEXT3 . " <br></div>");
    } else {
        // the actual date
        $data = date('Y-m-d');
        mysql_query("START TRANSACTION");
        $sql = "INSERT INTO mensagens (id_anuncios,users_id,for_user,mensagem,data) VALUES ($query,'" . $_SESSION['id'] . "','$id','$mens','$data')";
        $in = mysql_query($sql);
        if (($in)) {
            echo ("<p class=\"text-center\">" . LABEL_CONTACTAR_TEXT4 . " $nome_user_message " . LABEL_CONTACTAR_TEXT5 . "</p>");
            mysql_query("COMMIT");
        }
...
<html lang="en">
    ...
    <body>          
        ...
        <div class="container">
            <?php
            if (isset($nome_user_message)) {
                echo ("<a class=\"btn\" href=\"user.php?see=mensagem\">" . LABEL_CONTACTAR_TEXT7 . "</a><br><br>");
            }
            ?>
            <form class="form-signin" action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
                <?php
                if (isset($nome_user_message)) {
                    echo ("<textarea name=\"mensagem\" id=\"mensagem\" tabindex=\"1\" class=\"input-block-level\" placeholder=\"" . LABEL_CONTACTAR_TEXT8 . " $nome_user_message\" required></textarea>
					<p class=\"text-center\"><input class=\"btn btn-large btn-success\" type=\"submit\" name=\"submensagem\" value=\"" . LABEL_CONTACTAR_TEXT9 . "\"></p>");
                }
                ?>
            </form>
